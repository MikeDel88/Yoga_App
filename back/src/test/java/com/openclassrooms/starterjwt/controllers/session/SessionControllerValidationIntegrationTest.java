...
public class SessionControllerValidationIntegrationTest {
    ...
    private static Arguments invalidCase(String name, Consumer<SessionDto> mutation) {
        return Arguments.of(Named.of(name, mutation));
    }

    static Stream<Arguments> invalidSessionRequests() {
        return Stream.of(
                invalidCase("name is blank", sessionDto -> sessionDto.setName(" ")),
                invalidCase("name too long", sessionDto -> sessionDto.setName("n".repeat(51))),
                invalidCase("date is null", sessionDto -> sessionDto.setDate(null)),
                invalidCase("teacher_id is null", sessionDto -> sessionDto.setTeacher_id(null)),
                invalidCase("description is null", sessionDto -> sessionDto.setDescription(null)),
                invalidCase("description too long", sessionDto -> sessionDto.setDescription("d".repeat(2501)))
        );
    }
